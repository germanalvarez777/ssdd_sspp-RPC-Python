...
def call_remote_method(server_ip, operacion,op1,op2):
    try:
        calculadora_cliente = Pyro5.api.Proxy(f"PYRO:ejercicio2_rpc@{server_ip}:56792")
        if operacion == 1:
...
def correr_cliente():
    ...
    while (operacion == 1 or operacion == 2 or operacion == 3 or operacion == 4):
        ...
        with ThreadPoolExecutor() as executor:
            future = executor.submit(call_remote_method, server_ip, operacion,op1,op2)
